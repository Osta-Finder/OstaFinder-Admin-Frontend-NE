    ...
    <div className="bg-white rounded-2xl shadow-md border border-gray-100 p-6 lg:p-8 transition-all duration-300 hover:shadow-lg">
      ...
      <div className="grid grid-cols-3 gap-4 pt-6 border-t border-gray-100">
        <div className="text-center">
          ...
          <p className="text-lg font-bold text-gray-900">{Math.max(...computedData.map(d => d.value), 0).toLocaleString('en-US')} ج.م</p>
        </div>
        <div className="text-center">
          <p className="text-xs text-gray-500 mb-1">المتوسط</p>
          <p className="text-lg font-bold text-gray-900">{computedData.length > 0 ? Math.round(computedData.reduce((s, d) => s + d.value, 0) / computedData.length).toLocaleString('en-US') : 0} ج.م</p>
        </div>
        <div className="text-center">
          <p className="text-xs text-gray-500 mb-1">الأقل</p>
          <p className="text-lg font-bold text-gray-900">{Math.min(...computedData.map(d => d.value), 0).toLocaleString('en-US')} ج.م</p>
        </div>
      </div>
    </div>
  );
